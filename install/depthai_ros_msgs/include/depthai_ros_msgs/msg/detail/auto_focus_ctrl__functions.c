  ...
  depthai_ros_msgs__msg__AutoFocusCtrl__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    depthai_ros_msgs__msg__AutoFocusCtrl * data =
      (depthai_ros_msgs__msg__AutoFocusCtrl *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!depthai_ros_msgs__msg__AutoFocusCtrl__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          depthai_ros_msgs__msg__AutoFocusCtrl__fini(&data[i]);
        }
        free(data);
        return false;
      }
